...
- **移动端与性能专项（2026-09）**：触摸目标全面达标（≥44px）、底部安全区与键盘适配、分段控件统一；首屏主 bundle 909KB→409KB（gzip 293→133KB，-55%），KaTeX 统一单版本，字体自托管（离线可用、首屏零外部请求）
...
| **v1.3** | 工程化（进行中） | 首屏拆包（已完成，主 chunk -55%）· 字体自托管（已完成）· 移动端体验达标（已完成）· 架构解耦重构 · 命令注册表收敛 · 测试基建（fake-indexeddb）· ESLint 基线 |
