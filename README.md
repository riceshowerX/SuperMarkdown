...
| 痛点 | 竞品的困境 | SuperMarkdown 的解法 |
|------|-----------|---------------------|
| **图片管理糟糕** | Typora 删除图片留残渣、依赖图床、分享丢图 | 粘贴/拖拽图片自动转 **base64 内嵌**，单文件自包含、删除即干净、分享不丢图 |
| **学习曲线陡峭** | Obsidian 插件配置焦虑，被形容为"为吃羊角包学法语" | 打开即写、所见即所得、零配置 |
| **必须安装 / 必须注册** | Obsidian 无 Web 版、HackMD 须注册才能保存 | **Web 免安装免注册**，数据存浏览器本地 |
| **移动端集体拉胯** | 竞品移动端光标跳动、键盘消失、预览失效 | **响应式布局**，手机/平板直接可用，触控 ≥44px |
| **数据被锁定** | Notion 专有格式、数据在云端 | 数据主权在本地 IndexedDB，导出标准 .md / .html |
| **付费门槛** | Typora 89 元买断、Notion 订阅 | **完全免费**（MIT License） |
